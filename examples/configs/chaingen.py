...
name = "cn400"
topo = genTopo(name, 400)
if checkTopo(name, topo):
    with open(name + ".json", "w") as f:
        f.write(json.dumps(topo, indent=4))

# for index in range(0, 30):
#     name = "cn{}".format(index + 1)
#     node_num = 10 + 3 * index
#     router_num = node_num

#     link_lowerbound = node_num - 1
#     link_upperbound = math.floor(node_num * (node_num - 1) / (2 + (index + 1) * 18 / 30))
    
#     link_num = random.randint(link_lowerbound, link_upperbound)
#     topo = genTopo(name, node_num)
#     if checkTopo(name, topo):
#         with open(name + ".json", "w") as f:
#             f.write(json.dumps(topo, indent=4))